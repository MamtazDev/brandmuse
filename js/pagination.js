(function (root) {
  ...
  Pager.prototype.renderButton = function (obj) {
    ...
    if (obj.ellipsis) {
      ...
      item.item = true;
      link.href = o.hashString
        .replace("{page}", obj.page)
        .replace("{pages}", that.totalPages);
    }

    if (
      ((obj.prev || obj.first) && that.currentPage === 1) ||
...
trunc.addEventListener(
  ...
  (e) => {
    ...
    if (!val && parseInt(pages.value, 10) >= 1000) {
      shitTonsOfLinks = !confirm(
        "You have a shit-ton of pages!! Are you sure you want to disable truncation and render all the links?"
      );
    }
...
function changeSkin(e) {
  ...
  if (val === "bootstrap") {
    ...
    pager.config.firstText =
      '<span class="mdi mdi-chevron-double-left"></span>';
    pager.config.lastText =
